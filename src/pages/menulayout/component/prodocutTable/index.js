...
class ProductTable extends Component {
  ...
  constructor(props) {
    ...
    this.putawayProduct = this.putawayProduct.bind(this)
        ...
        <Menu.Item
          onClick={() => {
            Modal.confirm({
              ...
              cancelText: '否',
              onOk: () => {
                // this.putawayProduct('上架')
                message.success('产品上架成功')
              },
              onCancel() {
                message.warning('已取消上架')
              },
            });
          }}
        >
          ...
          onClick={() => {
            Modal.confirm({
              ...
              onOk: () => {
                ...
                // this.putawayProduct('下架')
                message.success('产品下架成功')

              }
              ,
              onCancel() {
                message.warning('已取消下架')
              },
            });
          }}>
          下架
  ...
  putawayProduct(str) {
    axios({
      method: "",
      url: `${base.url}/produce/modifyPutaway`,
      params: {
    ...
    axios.get(`${base.url}/produce/getProduce?currentPage=` + this.state.currentPage + `&limit=` + this.state.limit, {
      ...
      .then((res) => {
        ...
        if (!res.data.code == "SUCCESS") {
          ...
          this.setState({
            tableArr: res.data ? res.data.data.data : [],
            pagination: res.data ? res.data.data.pagination : []
          })
        }
      })
  }
